"""Legacy asset manager — подключение classic JS без участия Vite.

Что считается legacy в этом проекте
-----------------------------------
Это старые скрипты в духе jQuery 2.x, jQuery Migrate, плагины, IIFE-скрипты,
которые рассчитывают на синхронный порядок выполнения через обычные
``<script src="...">``. Такие файлы плохо дружат с ESM-pipeline Vite.

Схема работы
------------
1. Исходные legacy-файлы лежат внутри ``STATIC_ROOT``.
   Пути к ним задаются в манифесте относительно ``STATIC_ROOT``.
2. JSON-манифест с именованными модулями задаётся через ``LEGACY_MANIFEST_PATH``
   и может лежать где угодно.
3. Манифест поддерживает наследование (``extend``) и production-бандлы (``dist``).
4. В dev каждый файл подключается отдельно обычным classic ``<script>``.
5. В prod подключается один минифицированный бандл из ``dist``.

Итог: legacy-цепочка полностью отделена от Vite. Без полумагии, без blob-танцев,
без попыток заставить старый jQuery жить жизнью современного ESM-модуля.
"""
...
# Ключ в манифесте, содержащий список файлов для JS-модулей.
_FILES_KEY = "scripts"


class LegacyAssetManager:
    """Генерирует HTML-теги для подключения legacy-скриптов.

    Dev-режим:
        Возвращает отдельный ``<script>`` для каждого файла модуля
        с учётом наследования (``extend``).
        Файлы отдаются через FastAPI StaticFiles из ``STATIC_ROOT``.

    Prod-режим:
        Возвращает один ``<script>`` на production-бандл,
    ...
    def _manifest_path(self) -> Path:
        """Абсолютный путь до JSON-манифеста legacy-бандлов."""
        return Path(self._settings.legacy.MANIFEST_PATH).resolve()
    ...
    def render(self, module_name: str) -> Markup:
        """Рендерит теги для указанного legacy-модуля.

        Используется в шаблонах как ``{{ legacy_scripts('module') }}``.
        """
    ...
    def _render_dev(self, module_name: str) -> Markup:
        """Возвращает по одному classic ``<script>`` на каждый файл модуля.
    ...
    def _render_prod(self, module_name: str) -> Markup:
        ...
        if not dist_path:
            raise AssetManifestError(
                f"Legacy-модуль {module_name!r} не содержит ключа 'dist' — "
                f"невозможно определить путь к production-бандлу."
            )

        bundle_url = f"{self._static_url}/{dist_path}"
        return Markup(f'<script src="{bundle_url}"></script>')
