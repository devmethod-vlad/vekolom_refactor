...
- Per-service logging:
  - `VEKOLOM_LOG_TO_FILE`, `VEKOLOM_LOG_LEVEL`, `VEKOLOM_ACCESS_LOG_ENABLED`
...
- Для web-сервиса `vekolom` runtime-логи разделены:
  - `app.log` — все не-access логи (`root`, `app.*`, `uvicorn.error`, `gunicorn.error`);
  - `access.log` — только HTTP access-логи (`uvicorn.access`, `gunicorn.access`).
- Access-логирование управляется отдельно флагом `VEKOLOM_ACCESS_LOG_ENABLED` и по умолчанию выключено.
- При `VEKOLOM_ACCESS_LOG_ENABLED=false` access-логи полностью отключены (нет ни в stdout, ни в файле).
- При `VEKOLOM_LOG_TO_FILE=false` любые логи пишутся только в stdout.
...
- `${HOST_LOG_ROOT}/vekolom:/vekolom/logs` → `/vekolom/logs/app.log` и (опционально) `/vekolom/logs/access.log`
